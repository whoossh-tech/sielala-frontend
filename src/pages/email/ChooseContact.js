    ...
    <body>
      ...
      <main style={{ marginLeft: "60px" }}>
        <div className="bg-neutral-100 relative" style={{ backgroundImage: `url(${backgroundPhoto})`, backgroundSize: "cover", height: "150px" }}>
          ...
          <div>
            <p className="subtitle">
              ...
              /
              <a onClick={handleBackEmailList} style={{ borderBottom: "1px solid #E685AE", textDecoration: "none", cursor: "pointer" }}>
                {" "}
                Bulk Email List{" "}
              </a>{" "}
              /
        ...
        <div className="content-container my-4">
          ...
          <div className="button-field mx-8">
            <button className="button-grey" onClick={handleBackButton} disabled={isLoading}>
              Back
            </button>
            <button className="button-pink montserrat" type="submit" disabled={isLoading} onClick={sendEmailConfirmation}>
          ...
          <div style={{ marginBottom: "10px" }}>
            <p>
              <b>Contacts to be sent: {selectedContact.length}</b>
            </p>
          </div>
          ...
          <div className="mb-3" style={{ display: "flex", justifyContent: "center" }}>
                ...
                <tbody>
                  {contacts && contacts.length > 0 ? (
                    contacts.map((contact, i) => (
                      <tr key={i}>
                        <td>
                          <input type="checkbox" id={`checkbox-${i}`} name={`checkbox-${i}`} onChange={(e) => handleCheckboxChange(e, contact.idContact)} checked={selectedContact.includes(contact.idContact)} />
                        </td>
                        <td>{contact.name}</td>
                        ...
                        {contact.type === "Tenant" && (
                          <td className="text-secondary-80">
                            <b>Tenant</b>
                          </td>
                        )}

                        {contact.type === "Sponsor" && (
                          <td className="text-tertiary-80">
                            <b>Sponsor</b>
                          </td>
                        )}
                      </tr>
            ...
            <Modal isOpen={isModalOpen} onRequestClose={closeModal} id="modal-confirmation-form">
              ...
              <button className="button-green text-center" onClick={handleSendEmail}>
                Confirm
              </button>
            </Modal>
          </div>
