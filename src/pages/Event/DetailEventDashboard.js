            ...
            <div key={i} className="detailevent1-card bg-primary-10 shadow-md rounded-lg py-3 px-8 my-5" style={{ backgroundColor: "#EBF0B0", alignContent: "center" }}>
              <div className="flex justify-between">
                <div className="text-center">
                  ...
                  <p className="text-md mb-2 text-center">{event.description}</p>
        ...
        {eventData.map((event, i) =>
          event?.listTenant && isEventWithinSevenDays(event.startDateFormatted) && event.listTenant.length > 0 ? (
            event.listTenant.map((tenant, j) => (
              <div key={j} className="previousevent-card bg-primary-10 shadow-md rounded-lg py-3 px-8 my-5" style={{ backgroundColor: "#E9D8CB" }}>
                ...
                <p className="text-md mb-2 text-start">
                  ...
                  <a href={`https://www.instagram.com/${tenant.brandInstagram}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "underline" }}>
                    {tenant.brandInstagram}
                  </a>
                </p>
                <p className="text-md mb-2 text-start">🛒 {tenant.category}</p>
                <p className="text-md mb-2 text-start">💰 {tenant.brandPromo}</p>
              </div>
            ))
          ) : (
            <div style={{ backgroundColor: "#FFB2D3", borderRadius: "20px", padding: "10px", display: "inline-block", textAlign: "center" }} className="rounded my-4 px-2">
              <p>
                <b></b>Our eagerly anticipated tenant list is coming soon! Come and join the fun as Tenant!{" "}
              </p>
            </div>
          )
        )}
...
export default DetailEventDashboard;
