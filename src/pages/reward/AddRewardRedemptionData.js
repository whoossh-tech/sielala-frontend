...
const AddRewardRedemptionData = () => {
  ...
  useEffect(() => {
    ...
    // fetch event data
    axios
      .get(`${url}/api/reward/${idEvent}`)
      .then((res) => {
        setEventData(res.data.eventData);
      })
      .catch((err) => console.error("Error fetching event information:", err));

    // fetch list visitor
    axios
      .get(`${url}/api/visitor/view-all/${idEvent}`)
      .then((res) => {
    ...
    axios
      .get(`${url}/api/reward/view-reward-category-1/${idEvent}`)
      .then((res) => {
      ...
      .catch((err) => console.error("Error fetching reward cat1 data:", err));

    // fetch list reward cat 2
    axios
      .get(`${url}/api/reward/view-reward-category-2/${idEvent}`)
      .then((res) => {
        const temp = res.data.data.map((reward) => ({
          option: `${reward.productName} ${reward.brandName}`,
          idProduct: `${reward.idProduct}`,
      ...
      .catch((err) => console.error("Error fetching reward cat2 data:", err));
  }, [idEvent]);
  ...
  function getSuggestions(value) {
    return listVisitor.filter((visitor) => visitor.eventPass.toLowerCase().includes(value.trim().toLowerCase()) || visitor.name.toLowerCase().includes(value.trim().toLowerCase()));
  }
  ...
  function renderSuggestion(suggestion) {
    return (
      <span>
        {suggestion.eventPass} : {suggestion.name}
      </span>
    );
  }
  ...
  return (
    <body>
      ...
      <main style={{ marginLeft: "60px" }}>
        ...
        <form className="flex flex-row items-center pt-8 pb-20 mt-8 w-full text-neutral-100 bg-white rounded-2xl shadow-lg" onSubmit={(e) => onSubmit(e)}>
          <div className="flex flex-col space-y-4 pl-40">
              ...
              rouletteDataCat1.length === 0 ? (
                ...
                <div style={{ border: "none", zIndex: "0" }}>
                  <Wheel
                    ...
                    textDistance={55}
                    fontSize={[15]}
                    backgroundColors={["#F04684", "#F55151", "#F57A69", "#F2B05C", "#F4DB71", "#52D858", "#0CDAC6", "#5096F1", "#5668D5", "#C52CEE", "#818181"]}
            ...
            {errors.roulette && <span className="mt-0.5 text-danger text-xs">{errors.roulette}</span>}
          </div>
          ...
          <div className="flex flex-col items-stretch space-y-4 mt-6 w-full">
            ...
            <div className="input-form flex flex-col space-y-1">
              ...
              {errors.visitor && <span className="mt-0.5 text-danger text-xs">{errors.visitor}</span>}
            </div>
            ...
            <div className="input-form flex flex-col space-y-1">
              ...
              {errors.points && <span className="mt-0.5 text-danger text-xs">{errors.points}</span>}
            </div>
            ...
            <div className="input-form flex flex-col">
              ...
              <div className="relative overflow-clip w-full border border-neutral-40 rounded-lg">
                <input id="event" className="px-4 py-3 w-full focus:outline-none bg-gray-100" value={eventData.eventName} readOnly />
              </div>
            </div>
          ...
          <Modal isOpen={isConfirmationModalOpen} onRequestClose={closeConfirmationModal} id="modal-confirmation-form">
            ...
            <br></br>
            <div>
              <button className="button-red text-center" onClick={closeConfirmationModal}>
                Cancel
              </button>
          ...
          <Modal isOpen={isRewardModalOpen} onRequestClose={closeRewardModal} id="modal-reward">
            ...
            <br></br>
            <div>
              <button className="button-green text-center" onClick={handleClaim}>
                Claim
              </button>
            </div>
          </Modal>
