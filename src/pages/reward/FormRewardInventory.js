      ...
      <tr key={index}>
        ...
        <td>
          <div className={`overflow-clip w-100 border border-neutral-40 rounded-lg ${errors.listDayReward && "border-danger"}`}>
            <input className="px-4 py-3 w-full focus:outline-none" type="number" value={listDayReward[index]?.stokAwal || ""} min="0" disabled={index + 1 < day} onChange={(e) => handleDayStockChange(index, parseInt(e.target.value))} />
          </div>
        </td>
      </tr>
    ));
