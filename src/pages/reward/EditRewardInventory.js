      ...
      <tr key={dayReward.id}>
        ...
        <td>
          <div className={`overflow-clip w-100 border border-neutral-40 rounded-lg ${errors.listDayReward && "border-danger"}`}>
            <input className="px-4 py-3 w-full focus:outline-none" type="number" value={dayReward.stokAwal || ""} min="0" disabled={dayReward.day < day} onChange={(e) => handleDayStockChange(dayReward.day, parseInt(e.target.value))} />
          </div>
        </td>
      </tr>
    ));
          ...
          <div className="flex flex-col items-stretch space-y-4 mt-6 w-full">
            ...
            <div className="input-form flex flex-col space-y-1">
              ...
              <div className={`overflow-clip w-full border border-neutral-40 rounded-lg ${errors.brand_name && "border-danger"}`}>
                <input id="brand_name" className="px-4 py-3 w-full focus:outline-none" placeholder="Insert Brand Name" value={brandName} onChange={(e) => setBrandName(e.target.value)} />
              </div>
            ...
            <div className="input-form flex flex-col space-y-1">
              ...
              <div className={`overflow-clip w-full border border-neutral-40 rounded-lg ${errors.product_name && "border-danger"}`}>
                <input id="product_name" className="px-4 py-3 w-full focus:outline-none" placeholder="Insert Product Name" value={productName} onChange={(e) => setProductName(e.target.value)} />
              </div>

              {errors.product_name && <span className="mt-0.5 text-danger text-xs">{errors.product_name}</span>}
            </div>
            ...
            <div className="input-form flex flex-col">
              ...
              <div className={`relative overflow-clip w-full border border-neutral-40 rounded-lg ${errors.brand_category && "border-danger"}`}>
                <select id="category" className="appearance-none px-4 py-3 w-full focus:outline-none" placeholder="Choose category" value={category} onChange={(e) => setCategory(e.target.value)}>
              ...
              {errors.category && <span className="mt-0.5 text-danger text-xs">{errors.category}</span>}
            </div>
            ...
            <div className="input-form flex flex-col">
              ...
              <table className="max-w-lg">
                ...
                <tfoot>
                  <tr>
                    <td colSpan="2">{errors.listDayReward && <span className="mt-0.5 text-danger text-xs">{errors.listDayReward}</span>}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
