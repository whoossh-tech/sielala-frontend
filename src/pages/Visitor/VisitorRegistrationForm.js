    ...
    <main className="relative overflow-y-auto h-screen w-screen bg-neutral-10 select-none">
      ...
      <div className="bg-neutral-100 relative" style={{ backgroundImage: `url(${backgroundPhoto})`, backgroundSize: "cover", height: "200px" }}>
        <div>
          ...
          <div>
            <p className="subtitle" style={{ paddingLeft: 150 }}>
              Register as Visitor at Lala Market now!
            </p>
          </div>
        </div>
      </div>
      ...
      <form className="flex flex-col items-center px-4 pt-8 pb-6 mt-3 w-full max-w-screen-2xl mx-auto text-neutral-100 rounded-2xl" onSubmit={(e) => onRegister(e)}>
        ...
        <div className="input-form-visitor flex flex-col space-y-1 mt-2">
          ...
          <div className={`overflow-clip border border-neutral-40 rounded-lg ${errors.name && "border-danger"}`}>
            <input id="name" className="px-4 py-3 w-full focus:outline-none" placeholder="ex. Lala Market" value={name} onChange={(e) => setName(e.target.value)} />
          </div>

          {errors.name && <span className="mt-0.5 text-danger text-xs">{errors.name}</span>}
        </div>
        ...
        <div className="input-form-visitor flex flex-col space-y-1">
          ...
          <div className={`overflow-clip w-full border border-neutral-40 rounded-lg ${errors.email && "border-danger"}`}>
            <input id="email" className="px-4 py-3 w-full focus:outline-none" placeholder="ex. jane.doe@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>

          {errors.email && <span className="mt-0.5 text-danger text-xs">{errors.email}</span>}
        </div>
        ...
        <div className="input-form-visitor flex flex-col">
          ...
          {errors.telephone && <span className="mt-0.5 text-danger text-xs">{errors.telephone}</span>}
        </div>
        ...
        <div className="input-form-visitor flex flex-col">
          ...
          {errors.location && <span className="mt-0.5 text-danger text-xs">{errors.location}</span>}
        </div>
        ...
        <div className="input-form-visitor flex flex-col">
          ...
          <div className={`overflow-clip w-full border border-neutral-40 rounded-lg ${errors.age && "border-danger"}`}>
            <input id="age" type="telephone" className="px-4 py-3 w-full focus:outline-none" placeholder="ex. 20" value={age} onChange={(e) => setAge(e.target.value)} />
          </div>

          {errors.age && <span className="mt-0.5 text-danger text-xs">{errors.age}</span>}
        </div>
        ...
        <div className="input-form-visitor flex flex-col">
          ...
          {errors.gender && <span className="mt-0.5 text-danger text-xs">{errors.gender}</span>}
        </div>
